...
function SuccessfulBooking({ onClose, userId }) {
  ...
  useEffect(() => {bookingServices.getBooking(userId).then((result) => setData(result))}, []);
    ...
    <Modal show={true} onHide={onClose} centered>
      ...
      <Modal.Body>
        <p>
          ...
          has been made. 
          Your special request: {data.specialRequest}
          In case of delay more than 15 minutes, please contact us.
        </p>
      </Modal.Body>
      <Modal.Footer>
        <Button variant="primary" onClick={onClose} className={styles.whiteColor}>Close
        </Button>
      </Modal.Footer>
    </Modal>
  );
